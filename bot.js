...
client.on("ready", () => {
  ...
  client.user.setActivity('+yardım | 𐌞anity • #2021 ');
})
...
client.on("guildMemberRemove", async member => {
  ...
  if (!mesaj) {
    return client.channels.get(kanal).send(" Kullanıcı Ayrıldı. `" + sayaç + "` Kişi Olmamıza `" + sonuç + "` Kişi Kaldı `" + member.guild.memberCount + "` Kişiyiz!" + "`" + member.user.username + "`");
      }
